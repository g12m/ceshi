...
class LoginController extends HomeBaseController
{
    ...
    public function passwordReset()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $validate->message([
                // 'verification_code.require' => '短信验证码不能为空',
                'password.require'          => '密码不能为空',
                'password.max'              => '密码不能超过32个字符',
                'password.min'              => '密码不能小于6个字符',
                'captcha.require'           => '验证码不能为空',
            ...
            $list=Db::name('user')->field('id')->where('mobile',$data['username'])->find();
            $pass=cmf_password($data['password']);


            $res=Db::name('user')->where('id',$list['id'])->update(['user_pass'=>$pass]);

           if($res)
           {
             $this->success('修改成功',url('user/login/index.html'));
           }
           else {
               $this->error('修改失败');
           }
