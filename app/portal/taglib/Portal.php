...
?>
  <a href="{\$data['url']}" target="_blank"><img onclick="addclick({\$data['id']})" src="{:cmf_get_image_url(\$data['img'])}" width="{\$data['width']}"></a>
  <script>
  function addclick(id)
  {
    $.get("{:url('api/poster/addClick','',false)}/id/"+id)
  }
  </script>
parse;
      return $parse;
    ...
    */
    public function tagPosition($tag,$content)
    {
      ...
      $order         = empty($tag['order']) ? 'post.listorder DESC' : $tag['order'];
