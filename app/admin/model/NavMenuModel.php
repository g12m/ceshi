...
class NavMenuModel extends Model
{
    ...
    public function selectNavs()
    {
        ...
        foreach ($navs as $key => $navData) {
            ...
            $tpl                = "<option value='\$rule' data-name='\$name' data-catid='\$id'>\$spacer\$name</option>";
    ...
    public function selectMyNavs()
    {
        ...
        foreach ($navs as $key => $navData) {
            $tree->init($navData['items']);
            $tpl                = "<option value='\$id' data-name='\$name' data-catid='\$id'>\$spacer\$name</option>";
            $html               = $tree->getTree(0, $tpl);
            $navs[$key]['html'] = $html;
        }
